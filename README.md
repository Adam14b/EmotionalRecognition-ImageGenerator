...
#### 2.3.3. Этап 2:
Важно проинтерпретировать и сравнить результаты всех моделей (F1-score для ```МКЭ```), а также сравнить ```МГПИ```(Baseline) и ```МГПИ```(MVP). В случае успеха(результаты удовлетворяют бизнес требованиям), следует отправить их заказчику на рассмотрение, в ином случае следует вернуться к <strong>2.3.1. Этап 0</strong>. 
...
#### 2.3.6. Этап 5:
Необходимо подготовить отчет с промежуточными результатами и демонстрацией модели заказчику. В случае одобрения результатов заказчиком, необходимо начинать подготовку пилота. 
