...
> - ```Docker``` - система контейнеров, позволяющая отделить приложение от общей инфраструктуры. Позволяет запускать множество 				приложений на одном хосте, избегая конфликотов и необходимости изменять конфигурацию оборудывания для каждого 		отдельновзятого приложения.
> - ```Хост``` - устройство, предоставляющее возможность запускать приложения удаленно в формате "клинет-сервер".
