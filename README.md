...
#### 2.3.2. Этап 2:
Важно проинтерпретировать и сравнить результаты всех моделей, а также сравнить ```МГПИ```(Baseline) и ```МГПИ```(MVP). В случае успеха(результаты удовлетворяют бизнес требованиям), следует отправить их заказчику на рассмотрение. Если результаты не удовлетворяют минимальным требованиям к моделям, следует вернуться к <strong>2.3.1. Этап 0</strong>. 
