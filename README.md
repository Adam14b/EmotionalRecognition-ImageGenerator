...
##### Этап 3:
- ```МКЭ```('обработка кадра'):
	- модель преобразует каждый 5 кадр в черно-белый формат с оттенками серого, для упрощения вычислений и уменьшения объема данных 		(обучающие данные были в черно-белом формате).
