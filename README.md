...
- ```МКЭ```(основная нагрузка - ```GPU```):
	- необходимо обрабатывать видеоконференции в разрешении '1920 x 1080' при 30 кадрах в секунду, таким образом пропускная способность ```GPU``` на одну конференцию должна быть $\approx$ 180мб в секунду;
	>  Рассчитывается по формуле: $1920 \cdot 1080 \cdot 3$байта = 6220800 байт $\approx$ 6мб на один кадр (180мб для 30 кадров), где:
